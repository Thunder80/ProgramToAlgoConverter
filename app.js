...
let inputArray;
let line = 1;
const error = () => `There is some syntax error in the given code in line no. ${line} or you have given a syntax which is too unconventional`;
function convert() {
	...
	inputArray = inputArray.filter((item) => item.trim() !== "" || item === "\n");
...
function evaluate(input){
	let steps = [1], output = "", stepCounter = 0;
	let blocks = [], tab = "", loops = [];
	let i;
	for(i = 0;i < input.length;i++)
	{
		...
		//functions
		else if((datatypes.includes(input[i]) && input[i+1] === "*" && validToken(input[i+2]) && input[i+3] == "(") || (datatypes.includes(input[i]) && validToken(input[i+1]) && input[i+2] == "("))
		{
			steps = 1;
			let name = input[i+1] == "*" ? input[i+2]:input[i+1];
			let parameters = input.slice(input.indexOf("(", i)+1, input.indexOf(")", i)).join(" ");
		...
		else if(datatypes.includes(input[i]))
		{
			...
			while(input[j] !== ";")
			{
				if(validToken(input[j]) && input[j+1] === "=")
				{
					...
					let end = endComma > endColon || endComma == -1?endColon:endComma;
					let expression = input.slice(j+2, end).join(" ");
					if(statement === "")
						statement += `SET ${input[j]} := ${expression}`;
					else
						statement += `, ${input[j]} := ${expression}`
					j = end;
				}
				else if(input[j] === "," || validToken(input[j]) || input[j] === "*" || input[j] === "[" || input[j] === "]")
				{
					j++;
				}
				else if(input[j] === "\n")
		...
		else if(validToken(input[i]) && input[i+1] === "->")
		{
			...
			if(end == -1)
				{console.log(i);return error();}
			output += `${tab}Step ${steps[stepCounter]}: SET ${input[i]}->${input[i+2]} := ${expression}\n`;
			i = end;
			console.log()
			steps[stepCounter]++;
		}
		...
		else if(validToken(input[i]) && input[i+1] == "+" && input[i+2] == "+")
		{
		...
		{
			if(validToken(input[i+1]))
				datatypes.push(input[i+1]);
			else
				{console.log(i);return error();}
			let j = i+2;
			...
			{
				j++;
				statement = `An ADT with the name of ${input[i+1]} has been declared with the following fields: `;
				let type = "";
				let comma = false;
				while(input[j] !== "}")
				{
		...
		else if(input[i] === "}")
		{
			...
			else if(block == "for")
			{
				...
				let pos = loops.pop();
				output = output.substring(0, pos) + steps[stepCounter] + output.substring(pos);
				steps.pop();
				stepCounter--;
				tab = "";
				for(let k = 0;k < stepCounter;k++)
			...
			{
				let pos = loops.pop();
				output = output.substring(0, pos) + (steps[stepCounter] - 1)+ output.substring(pos);
				steps.pop();
				stepCounter--;
				tab = "";
				for(let k = 0;k < stepCounter;k++)
		...
		{
			let condition = input.slice(input.indexOf("(", i)+1, input.indexOf(")", i)).join(" ");
		...
		else if(input[i] === "printf")
		{
			...
			if(comma < brac && comma != -1)
				output += `${tab}Step ${steps[stepCounter]}: PRINT ` + input.slice(comma+1, brac-1).join(" ") + "\n";
			else
				output += `${tab}Step ${steps[stepCounter]}: PRINT ` + input.slice(i+3, brac-2).join(" ") + "\n";
			steps[stepCounter]++;
			i = brac;
		}
		...
		{
			let brac = input.indexOf(")", i);
			let exp = input.slice(input.indexOf(",", i)+1, brac).join(" ").replace(/&/g, "");
	...
	for (i = 0; i < input.length; i++) {
		...
		//initializations, declarations
			...
			while (input[j] !== ";") {
				...
				if (j >= input.length || j == -1) { console.log(i); return error(); }
			}
			i = j;
			if (statement !== "") {
				output += `${tab}Step ${steps[stepCounter]}: ${statement}\n`;
		...
		//only initialization
			...
			let end = input.indexOf(";", i + 2);
			let expression = input.slice(i + 2, end).join(" ");
			if (end == -1) { console.log(i); return error(); }
			output += `${tab}Step ${steps[stepCounter]}: SET ${input[i]} := ${expression}\n`;
			i = end;
			steps[stepCounter]++;
		}
		...
		//structures
			...
			if (validToken(input[i + 1]))
				datatypes.push(input[i + 1]);
			else { console.log(i); return error(); }
			let j = i + 2;
			let declaration = false;
			while (input[j] != "{") {
				if (input[j] !== "\n") {
...
>>>>>>> 991c7645609ef1b49d7c097667cb1591e62313b4
